...
## 引言
本文档面向嵌入式KV/TLV存储系统NanoKV，这是一个全新的轻量级实现，替代了原有的MicroKV。NanoKV专注于三大核心算法：追加写入机制（状态机+CRC）、LFU缓存（访问计数与替换策略）、增量垃圾回收（分摊与进度控制）。文档从代码级实现出发，结合类图、时序图、流程图与复杂度分析，帮助读者理解算法设计、协作关系与性能优化要点，并提供伪代码与实际代码片段路径以便进一步查阅。

## 项目结构
NanoKV采用极简文件架构，核心逻辑集中在单一C文件中，配合头文件与配置文件组织功能模块。移植层负责Flash抽象，便于在不同MCU/平台复用。
...
- [NanoKV.c](file://NanoKV.c#L251-L426)
- [NanoKV.c](file://NanoKV.c#L1178-L1501)
...
**图表来源**
- [NanoKV.c](file://NanoKV.c#L890-L910)
- [NanoKV.c](file://NanoKV.c#L59-L70)
...
**章节来源**
- [NanoKV.c](file://NanoKV.c#L592-L735)
- [NanoKV.h](file://NanoKV.h#L170-L180)
...
- [NanoKV.c](file://NanoKV.c#L381-L425)
...
**图表来源**
- [NanoKV.c](file://NanoKV.c#L1178-L1501)
- [NanoKV.c](file://NanoKV.c#L428-L476)
- [NanoKV.c](file://NanoKV.c#L1405-L1450)
...
**章节来源**
- [NanoKV.h](file://NanoKV.h#L44-L53)
- [NanoKV_port.c](file://NanoKV_port.c#L54-L88)
- [NanoKV.c](file://NanoKV.c#L843-L932)
- [NanoKV.c](file://NanoKV.c#L933-L991)
...
### 伪代码示例与代码片段路径
- **追加写入（状态机+CRC）**
  - 写入流程：[写入条目与状态更新](file://NanoKV.c#L890-L910)
  - CRC计算：[CRC16计算](file://NanoKV.c#L59-L70)
- **LFU缓存**
  - 命中/替换：[缓存查找与LFU候选](file://NanoKV.c#L128-L161)
  - 更新缓存：[缓存更新](file://NanoKV.c#L164-L192)
- **增量GC**
  - 步进执行：[增量GC一步](file://NanoKV.c#L654-L718)
  - 启动与进度：[启动GC与擦除源扇区](file://NanoKV.c#L612-L718)
- **多扇区环形**
  - 切换扇区：[切换到指定/下一扇区](file://NanoKV.c#L381-L425)
  - 扫描写入偏移：[扫描写入偏移](file://NanoKV.c#L267-L330)
- **TLV历史与保留**
  - 历史排序：[按写入顺序排序](file://NanoKV.c#L1430-L1441)
  - 保留阈值：[准备保留阈值](file://NanoKV.c#L448-L460)