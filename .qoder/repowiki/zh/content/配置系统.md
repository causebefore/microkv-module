...
## 简介
本文档系统性梳理 NanoKV 3.0 的编译时配置体系，这是一个全新的轻量级嵌入式KV/TLV存储库实现。NanoKV专注于以下关键配置项及其影响：
- 键值长度限制：键名与值的最大长度
- 缓存配置：是否启用、缓存条目数量
...
- 可靠性增强配置
  - 读取时CRC校验：NKV_VERIFY_ON_READ（默认 1 启用）
  - 启动时清理WRITING状态：NKV_CLEAN_DIRTY_ON_BOOT（默认 1 启用）
...
```mermaid
graph TB
cfg["编译期配置<br/>NKV_MAX_KEY_LEN/NKV_MAX_VALUE_LEN<br/>NKV_CACHE_ENABLE/NKV_CACHE_SIZE<br/>NKV_INCREMENTAL_GC/NKV_GC_ENTRIES_PER_WRITE/NKV_GC_THRESHOLD_PERCENT<br/>NKV_TLV_RETENTION_ENABLE/NKV_TLV_RETENTION_MAX<br/>NKV_VERIFY_ON_READ/NKV_CLEAN_DIRTY_ON_BOOT<br/>NKV_DEBUG_ENABLE"] --> api["API 层<br/>nkv_set/nkv_get/nkv_del/nkv_scan/nkv_format"]
api --> impl_core["核心实现<br/>KV/TLV 存储、CRC、对齐、条目大小计算"]
impl_core --> gc["增量GC/全量GC"]
impl_core --> cache["LFU缓存"]
impl_core --> tlv["TLV保留策略"]
impl_core --> verify["CRC校验"]
...
**章节来源**
- [NanoKV_cfg.h](file://NanoKV_cfg.h#L11-L12)
...
**章节来源**
- [NanoKV_cfg.h](file://NanoKV_cfg.h#L18-L19)
...
**章节来源**
- [NanoKV_cfg.h](file://NanoKV_cfg.h#L22-L24)
...
**章节来源**
- [NanoKV_cfg.h](file://NanoKV_cfg.h#L27-L28)
- [NanoKV.h](file://NanoKV.h#L256-L259)
- [NanoKV.c](file://NanoKV.c#L35-L52)
- [NanoKV.c](file://NanoKV.c#L429-L476)
- [NanoKV.c](file://NanoKV.c#L1463-L1501)

### 可靠性增强配置
- 参数
  - NKV_VERIFY_ON_READ：读取时CRC校验（默认 1 启用）
  - NKV_CLEAN_DIRTY_ON_BOOT：启动时清理WRITING状态（默认 1 启用）
- 作用与影响
  - 启用CRC校验可确保数据完整性，防止读取错误
  - 启动时清理WRITING状态可修复掉电异常导致的数据不一致
- 取值范围与默认值
  - 开关：0/1（默认 1）
- 相互依赖
  - 与配置版本管理机制协同工作
- 性能与可靠性
  - CRC校验会增加读取开销，但显著提升数据可靠性
  - 启动清理会增加初始化时间，但确保系统一致性
- 常见错误
  - CRC校验失败导致读取错误
  - 启动清理影响系统启动速度
- 优化建议
...
**章节来源**
- [NanoKV_cfg.h](file://NanoKV_cfg.h#L31-L32)
- [NanoKV.c](file://NanoKV.c#L314-L320)
- [NanoKV.c](file://NanoKV.c#L956-L984)
...
**章节来源**
- [NanoKV_cfg.h](file://NanoKV_cfg.h#L35-L89)
...
## 结论
NanoKV 3.0 的配置系统围绕"键值长度、缓存、GC 策略、TLV 保留策略、可靠性增强、调试输出"六大维度展开，既保证了嵌入式环境下的资源友好性，又提供了灵活的性能调优空间。通过合理设置参数并结合场景化的优化策略，可在内存受限设备、高吞吐量应用与实时系统等不同场景下取得良好表现。新增的可靠性增强功能进一步提升了系统的稳定性和数据完整性。
...
- 内存受限设备（RAM < 1KB）
  ...
  - 可靠性：NKV_VERIFY_ON_READ=1，NKV_CLEAN_DIRTY_ON_BOOT=1
- 高吞吐量应用（频繁读写）
